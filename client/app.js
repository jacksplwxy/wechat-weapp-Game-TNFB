...
App({
  ...
  appData: {
    ...
    tunnel_status: 'close',//统一管理唯一的信道连接的状态：connect、close、reconnecting、reconnect、error
  },
  onLaunch(opt) {
  ...
  tunnel_create() {//创建一个新信道，并监听相关数据的变化
    const that = this
    const tunnel = that.tunnel = new qcloud.Tunnel(config.service.tunnelUrl);
    tunnel.open();
    tunnel.on('connect', () => {//监听信道连接
      console.info("tunnel_status = 'connect'")
      this.appData.tunnel_status = 'connect' //改变信道状态为已连接
      if (that.tunnelConnectCallback) {//设置回调
        that.tunnelConnectCallback()
      }
    })
    tunnel.on('close', () => {//监听信道断开
      console.info("tunnel_status = 'close'")
      this.appData.tunnel_status = 'close' //改变信道状态为已断开
      if (that.tunnelCloseCallback) {//设置回调
        that.tunnelCloseCallback()
      }
    })
    tunnel.on('reconnecting', () => {//监听信道重新链接
      console.info("tunnel_status = 'reconnecting'")
      this.appData.tunnel_status = 'reconnecting' //改变信道状态为重新连接中
      if (that.tunnelReconnectingCallback) {//设置回调
        that.tunnelReconnectingCallback()
      }
    })
    tunnel.on('reconnect', () => {//监听信道重新连接成功
      console.info("tunnel_status = 'reconnect'")
      this.appData.tunnel_status = 'reconnect' //改变信道状态为重新连接成功
      if (that.tunnelReconnectCallback) {//设置回调
        that.tunnelReconnectCallback()
      }
    })
    tunnel.on('error', () => {//监听信道发生错误
      console.info("tunnel_status = 'error'")
      this.appData.tunnel_status = 'error' //改变信道状态为发生错误
      if (that.tunnelErrorCallback) {//设置回调
        that.tunnelErrorCallback()
      }
    })
    tunnel.on('has_matched', (res) => {//监听匹配成功
      if (that.tunnelHas_matchedCallback) {//设置回调
        that.tunnelHas_matchedCallback(res)
      }
    })
  },
  ...
  */
  ...
  storeUser_network(opt) {
    ...
    function store(that, userInfo, opt, GId = '') {
      let data = {
        //clickId:自动生成的数据,
